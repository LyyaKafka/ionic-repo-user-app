...
import { Firestore, collectionData, doc, docData, addDoc, updateDoc, deleteDoc, collection, documentId, setDoc, getDocs, query, where } from '@angular/fire/firestore';
...
// ----------------------------
  ...
  async addCurrentUser(username: string, email: string, userid: any){
      ...
      return await setDoc(userRef, {username: username, email: email}).catch(Error => {
  ...
  async getDocumentsByUserId(uid: string) {
    const q = query(collection(this.firestore, "documents"), where("ownerid", "==", uid));
  ...
  async getDocuments() {
    const q = query(collection(this.firestore, "documents"), where("visibility", "==", true));
    const snapshot = await getDocs(q);
    return snapshot;
  }
  ...
  async addDocument(document: Document): Promise<Boolean>{
    const docRef = await addDoc(collection(this.firestore, 'documents'), document);

    const userDocRef = this.addUserDocument({ documentid: docRef.id }, document.ownerid);
...
// ----------------------------------
  ...
  async foo1(){
    ...
    // const q = query(collection(this.firestore, "documents"), where("ownerid", "==", "aGcqt4Tnx9VgtqATyB2DSx8eIrP2"));
    const q = query(collection(this.firestore, "documents"), where("visibility", "==", true));
